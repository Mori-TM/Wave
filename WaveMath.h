...
#if (defined(__x86_64__) || defined(_M_X64) || defined(__i386__) || defined(_M_IX86)) && !defined(WAVE_CROSS_PLATFORM)
...
#if defined(WAVE_HAS_AVX)
typedef __m256 Wave256;
#else
typedef float Wave256[8];
#endif
...
void Normalize4P(vec4* v)
{
	...
	v->Fast = _mm_blendv_ps(_mm_div_ps(v->Fast, _mm_sqrt_ps(len_sq)), v->Fast, is_zero);
#elif defined(WAVE_HAS_NEON)
	...
	v->Fast = vbslq_f32(is_zero, v->Fast, vdivq_f32(v->Fast, len));
#else
...
WAVE_INLINE vec4 Mul4fP(vec4* a, float b)
{

#if defined(WAVE_HAS_SSE)
	vec4 r;
	r.Fast = _mm_mul_ps(a->Fast, _mm_set_ps1(b));
	return r;
#elif defined(WAVE_HAS_NEON)
	vec4 r;
	r.Fast = vmulq_f32(a->Fast, vdupq_n_f32(b));
	return r;
#else
	vec4 r;
	r.x = a->x * b;
...
WAVE_INLINE vec4 Mul4f(vec4 a, float b)
{
	return Mul4fP(&a, b);
}
...
WAVE_INLINE void Vec4P(float x, float y, float z, float w, Wave128* r)
{
	...
	*r = vcombine_f32(xy, zw);
	...
	a.z = z;
	a.w = w;
	memcpy(r, a.Fast, sizeof(Wave128));
#endif
}
...
mat4 MultiplyMat4P(mat4* a, mat4* b)
{
	...
	{
		Wave128 c = b->Fast[i];
		Wave128 t = vmulq_f32(a->Fast[0], vdupq_laneq_f32(c, 0));
...
WAVE_INLINE mat4 MultiplyMat4(mat4 a, mat4 b)
{
...
WAVE_INLINE vec4 MultiplyVec4Mat4(vec4 v, mat4 m)
{
	return MultiplyVec4Mat4P(&v, &m);
}
...
mat4 LookAtMat4PFast(vec4* Pos, vec4* LookAt, vec4* Up)
{
	mat4 Matrix;

	vec4 Z = Normalize4(Sub4P(LookAt, Pos));
	vec4 X = Normalize4(Cross4P(&Z, Up));
	vec4 Y = Cross4P(&X, &Z);

	Z = Mul4fP(&Z, -1);

	Vec4P(X.x, Y.x, Z.x, 0.0, &Matrix.m4[0]);
	Vec4P(X.y, Y.y, Z.y, 0.0, &Matrix.m4[1]);
	Vec4P(X.z, Y.z, Z.z, 0.0, &Matrix.m4[2]);
	Vec4P(-Dot4P(&X, Pos), -Dot4P(&Y, Pos), -Dot4P(&Z, Pos), 1.0, &Matrix.m4[3]);

	return Matrix;
}

mat4 LookAtMat4Fast(vec4 Pos, vec4 LookAt, vec4 Up)
{
	return LookAtMat4PFast(&Pos, &LookAt, &Up);
}
...
void PerspectiveMat4PFast(float Aspect, float FOV, float NearPlane, float FarPlane, mat4* m)
{
	register float F = 1.0 / tanf(0.5 * FOV);

	Vec4P(F / Aspect, 0.0, 0.0, 0.0, &m->m4[0]);
	Vec4P(0.0, -F, 0.0, 0.0, &m->m4[1]);
	Vec4P(0.0, 0.0, FarPlane / (NearPlane - FarPlane), -1.0, &m->m4[2]);
	Vec4P(0.0, 0.0, (NearPlane * FarPlane) / (NearPlane - FarPlane), 0.0, &m->m4[3]);
}


mat4 PerspectiveMat4Fast(float Aspect, float FOV, float NearPlane, float FarPlane)
{
	mat4 m;
	PerspectiveMat4PFast(Aspect, FOV, NearPlane, FarPlane, &m);
	return m;
}

void OrthoMat4P(float Left, float Right, float Bottom, float Top, float NearZ, float FarZ, mat4* m)
{
	Vec4P(2 / (Right - Left), 0.0, 0.0, 0.0, &m->m4[0]);
	Vec4P(0.0, 2 / (Top - Bottom), 0.0, 0.0, &m->m4[1]);
	Vec4P(0.0, 0.0, -1 / (FarZ - NearZ), 0.0, &m->m4[2]);
	Vec4P(-(Right + Left) / (Right - Left), -(Top + Bottom) / (Top - Bottom), -NearZ / (FarZ - NearZ), 1.0, &m->m4[3]);
}
...
void MulMat4FloatP(mat4* m, float f)
{
	m->m4[0] = Mul4fP(&m->m4[0], f);
	m->m4[1] = Mul4fP(&m->m4[1], f);
	m->m4[2] = Mul4fP(&m->m4[2], f);
	m->m4[3] = Mul4fP(&m->m4[3], f);
}

WAVE_INLINE mat4 MulMat4Float(mat4 m, float f)
{
	MulMat4FloatP(&m, f);
	return m;
}

WAVE_INLINE void SetMat4Vec4P(vec4* a, vec4* b, vec4* c, vec4* d, mat4* Res)
{
#ifdef WAVE_CROSS_PLATFORM
	memcpy(Res->Fast, a, sizeof(vec4));
	memcpy(Res->Fast[1], b, sizeof(vec4));
	memcpy(Res->Fast[2], c, sizeof(vec4));
	memcpy(Res->Fast[3], d, sizeof(vec4));
#elif defined(WAVE_HAS_SSE)
	Res->Fast[0] = a->Fast;
	Res->Fast[1] = b->Fast;
	Res->Fast[2] = c->Fast;
	Res->Fast[3] = d->Fast;
...
//	_mm_storeu_ps(&Res->Fast[3], d->Fast);
	...
	Res->Fast[2] = c->Fast;
	Res->Fast[3] = d->Fast;
#endif // !WAVE_CROSSPLATFORM	
}


WAVE_INLINE mat4 SetMat4Vec4(vec4 a, vec4 b, vec4 c, vec4 d)
{
	mat4 Res;
	SetMat4Vec4P(&a, &b, &c, &d, &Res);
	return Res;
}
...
//	};
	...
	Vec4P(Mat->m[0][0], Mat->m[1][0], Mat->m[2][0], Mat->m[3][0], &Matrix.m4[0]);
	Vec4P(Mat->m[0][1], Mat->m[1][1], Mat->m[2][1], Mat->m[3][1], &Matrix.m4[1]);
	Vec4P(Mat->m[0][2], Mat->m[1][2], Mat->m[2][2], Mat->m[3][2], &Matrix.m4[2]);
	Vec4P(Mat->m[0][3], Mat->m[1][3], Mat->m[2][3], Mat->m[3][3], &Matrix.m4[3]);
